...
class Solution {
    ...
    fun solution(sizes: Array<IntArray>): Int {
         ...
         */
        input = sizes.map { it.toMutableList() }.toMutableList() // [1]
        getMaxValueAndColumn(input) // [2]
        if (maxValueColumn == 0) {
            input.sortWith(compareByDescending<MutableList<Int>> { it[0] }.thenByDescending { it[1] }) // [3]
            swapValueByCond(0, 1) // [4]
            val maxValueOtherColumn: Int = getMaxValueInOtherColumn(1) // [5]
            answer = maxValue * maxValueOtherColumn // [6]
        } else {
            input.sortWith(compareByDescending<MutableList<Int>> { it[1] }.thenByDescending { it[0] }) // [3]
            swapValueByCond(1, 0) // [4]
            val maxValueOtherColumn: Int = getMaxValueInOtherColumn(0) // [5]
            answer = maxValue * maxValueOtherColumn // [6]
        }
